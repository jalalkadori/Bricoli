  ...
  <body>
    <?php
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $chercheurEmail = sanitizeInput($_POST['chercheurEmail']);
            $oldPassword = sanitizeInput($_POST['old_password']);
            $newPassword = sanitizeInput($_POST['new_password']);

            // Validate the email adresse 
            if (empty($chercheurEmail)) {
                $errors['chercheurEmail'] = "Veuillez entrer votre adresse email.";
            } elseif (!filter_var($chercheurEmail, FILTER_VALIDATE_EMAIL)) {
                $errors['chercheurEmail'] = "L'adresse email n'est pas valide.";
            }

            // Check if the CIN exists in the database only if there are no previous validation errors
            if (empty($errorMessage['chercheurEmail'])) {
                // Check if the CIN exists in the database
                $stmt = $db_connection->prepare("SELECT * FROM `chercheur` WHERE email_chercheur = :email");
                $stmt->bindParam(':email', $chercheurEmail);
                $stmt->execute();
                $chercheur = $stmt->fetch(PDO::FETCH_ASSOC);

                if (!$chercheur) {
                    // Display an error message if the email adresse does not exist in the database
                    $errorMessage['chercheurEmail'] = "Adresse email introuvable. Veuillez resssayez.";
                }
            }

            if ($user) {
                // Check if the old password matches
                if (password_verify($oldPassword, $chercheur['mdp_chercheur'])) {
                    ...
                    if (strlen($newPassword) < 8) {
                        ...
                        $stmt = $db_connection->prepare("UPDATE chercheur SET mdp_chercheur = :password WHERE email_chercheur = :cin");
                        $stmt->bindParam(':password', $hashedPassword);
                        $stmt->bindParam(':email_chercheur', $email_chercheur);
                ...
                // Display an error message if the adresse email does not exist in the database
                $errorMessage['email_chercheur'] = "Adresse email introuvable. Veuillez resssayez.";
            }
        }
    ?>
            ...
            <section class="container center-content rounded-5 container-shadow mt-5 m-md-0 m-lg-0">
                <div class="row py-5 bg-white align-items-center">
                    ...
                    <div class="col-12 col-lg-8">
                        ...
                        <form method="POST">
                            <div class="mb-3">
                                <label for="chercheurEmail" class="form-label">Adresse mail actuellle :</label>
                                <input type="email" class="form-control border-dark" name="chercheurEmail" value="<?php echo htmlspecialchars($chercheurEmail); ?>">
                                <?php if (isset($errorMessage['chercheurEmail'])) echo '<span class="text-danger">' . $errorMessage['chercheurEmail'] . '</span>'; ?>
                            </div>
                            <div class="mb-3">
